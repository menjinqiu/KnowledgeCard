...
已修复一个同步完整性风险：新设备自动创建的空“当前常用”系统集合不应覆盖远端已有常用集合
已新增卡片目录引用安全检查：如果卡片 primaryDirectoryId 指向不存在的目录，立即同步和普通导入都应拦截
...
NOTE: 当前有未提交变更：src/services/syncPlanService.ts、src/services/importExportService.ts、CURRENT_STATE.md、NEXT_SESSION_PROMPT.md。此前同步 UI 简化已提交，当前新增的是同步完整性修复。
```
